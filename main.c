...
int ContientErreur(typejeton T[],SDL_Renderer *ren,SDL_Color red,TTF_Font *font2) {
    ...
    while (T[i].lexem != FIN) {
        if (T[i].lexem == ERREUR) {
            ...
            {
            case 0:
            printText(120, 17, "Carractere Inconnu", 200, 15, font2, red, ren);                
            break;
            case 1:
...
//extraction des bornes de l'ensemble de définition
...
if (ContientErreur(T,ren,red,font2) == 0) { 
 ...
 //-------------------------------------------------------------------------------tracé----------------------------------------------------------------------------------------
        ...
        printText(340, 17, "Veuillez reessayer", 200, 15, font2, red, ren);
        updateDisplay(ren);
        SDL_Delay(2000);
